class ErdController < ApplicationController
  ...
  def determine_association(first_table, second_table, foreign_key, association)
    second_table, association_type = determine_alias(association)
    relationship_type = determine_relationship(association)

    if relationship_type == "has_one"
      add_has_one(first_table, second_table, @primary_keys[second_table], foreign_key)
    elsif relationship_type == "has_many"
      add_has_many(first_table, second_table, @primary_keys[first_table], foreign_key)
    elsif relationship_type == "has_and_belongs_to_many"
      add_has_and_belongs_to_many(first_table, second_table, @primary_keys[first_table], foreign_key)
    elsif relationship_type == "belongs_to"
      add_belongs_to(first_table, second_table, @primary_keys[first_table], foreign_key)
    elsif relationship_type == "through"
      add_through(first_table, second_table, association)
    else
      @other << relationship
    end
  end
  ...
  def add_has_one(first_table, second_table, primary_key, foreign_key)
    fk = has_relationship_fk(foreign_key, first_table)
    @has_one << define_relationship(first_table, second_table, primary_key, fk)
  end

  def add_has_many(first_table, second_table, primary_key, foreign_key)
    fk = has_relationship_fk(foreign_key, first_table)
    @has_many << define_relationship(first_table, second_table, primary_key, fk)
  end

  def add_has_and_belongs_to_many(first_table, second_table, primary_key, foreign_key)
    fk = has_relationship_fk(foreign_key, first_table)
    @has_and_belongs_to_many << define_relationship(first_table, second_table, primary_key, fk)
  end

  def add_belongs_to(first_table, second_table, primary_key, foreign_key)
    unless second_table == "left_sides"
      fk = belongs_relationship_fk(foreign_key, second_table)
      @belongs_to << define_relationship(first_table, second_table, primary_key, fk)
    end
  end

  def add_through(first_table, second_table, association)
  ...
  def determine_join_associations
    @belongs_to.each {|belongs| examine_through_associations(belongs) }
  end

  def examine_through_associations(belongs)
